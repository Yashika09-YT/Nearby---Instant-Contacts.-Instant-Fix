...
function Index({ children }) {
  ...
  // Fetch categories and states from backend
  useEffect(() => {
    axios
      .get(API.categories)
      .then((res) => {
        if (res.data.success) {
          // categories come like { customer_id: "plumbing", name: "Plumbing" }
          dispatch({ type: 'SET_CATEGORIES', payload: res.data.data });
        }
      })
      .catch((e) => console.error(e));
    ...
    axios
      ...
      .then((res) => {
        if (res.data.success) {
          // states come like { state: "Gujarat" }
          dispatch({ type: 'SET_STATES', payload: res.data.data });
        }
      })
      .catch((e) => console.error(e));
  ...
  return (
    <div className='flex min-h-screen'>
      ...
      <div className='flex-1 min-h-screen bg-gray-100'>
          ...
          {!isHide && (
            <div className='flex'>
              ...
              <Select
                className='mr-2'
                value={search.category}
                onChange={(value) => fieldOnChange('category', value)}
              >
                <Option value='all'>Category (All)</Option>
                {data.categories.length > 0 &&
                  data.categories.map((category) => (
                    <Option
                      key={category.customer_id}
                      value={category.customer_id} // 👈 send customer_id like "plumbing"
                    >
                      {category.name}
                    </Option>
                  ))}
        ...
        <main className='p-4'>{children}</main>
        <footer className='text-center bg-white py-4'>
          Yashika Tanwar{new Date().getFullYear()} Created by Yashika Tanwar
        </footer>
      </div>
    </div>
  );
