...
//                   >
//                     {cityOption
//                       .filter((c) => c.stateId === field.state)
//                       .map((c) => (
//                         <Option key={c.id} value={c.id}>
//                           {c.city}
//                         </Option>
...
export default function AddServices() {
  ...
  const submitData = () => {
    ...
    const cityObj = cityOption.find((c) => c._id === city);
    const categoryObj = categoryOption.find((c) => c.id === type);
    const stateObj = stateOption.find((s) => s._id === stateId);
  ...
  //   if (name === 'state') setField({ ...field, state: value, city: undefined });
  // };
  const setFieldFn = (name, value) => {
  if (name === 'state') {
    // Reset city whenever state changes
    setField({ ...field, state: value, city: undefined });
  } else {
    setField({ ...field, [name]: value });
  }
};
  ...
  useEffect(() => {
    ...
    axios.get(API.getAllCity)
      .then((res) => {
        console.log('City API Response:', res.data); // <- ye add karo
              ...
              <Col span={10} offset={1}>
                <div style={{ marginBottom: '1rem' }}>
                  ...
                  <Select
                    ...
                    style={{ width: '100%', fontWeight: 'normal' }}
                  >
                    <Option key='91' value='India'>India</Option>
                  </Select>
                </div>
              </Col>
              <Col span={10} offset={1}>
              ...
              <Col span={10} offset={1}>
                <div style={{ marginBottom: '1rem' }}>
                  ...
                  >
                    {field.state &&
                      cityOption
                        .filter((c) => c.state_id === field.state)
                        .map((c) => (
...
>
  {field.state &&
    cityOption
      .filter((c) => String(c.state_id) === String(field.state))
      .map((c) => (
        ...
        <div className='flex justify-end mt-4'>
          <Button type='primary' loading={loader} onClick={submitData}>
            Submit
          </Button>
        </div>
      </div>
